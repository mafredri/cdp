...
// Package dom implements the DOM domain. This domain exposes DOM
// read/write operations. Each DOM Node is represented with its mirror
// object that has an `id`. This `id` can be used to get additional
// information on the Node, resolve it into the JavaScript object
// wrapper, etc. It is important that client receives DOM events only
// for the nodes that are known to the client. Backend keeps track of
// the nodes that were sent to the client and never sends the same node
// twice. It is client's responsibility to collect information about
// the nodes that were sent to the client.
//
// Note that `iframe` owner
// elements will return corresponding document elements as their child
// nodes.
package dom
...
// domainClient is a client for the DOM domain. This domain exposes
// DOM read/write operations. Each DOM Node is represented with its
// mirror object that has an `id`. This `id` can be used to get
// additional information on the Node, resolve it into the JavaScript
// object wrapper, etc. It is important that client receives DOM events
// only for the nodes that are known to the client. Backend keeps track
// of the nodes that were sent to the client and never sends the same
// node twice. It is client's responsibility to collect information
// about the nodes that were sent to the client.
//
// Note that `iframe`
// owner elements will return corresponding document elements as their
// child nodes.
...
// CollectClassNamesFromSubtree invokes the DOM method. Collects class
// names for the node with given id and all of it's child nodes.
...
// CopyTo invokes the DOM method. Creates a deep copy of the specified
// node and places it into the target container before the given
// anchor.
...
// DescribeNode invokes the DOM method. Describes node given its id,
// does not require domain to be enabled. Does not start tracking any
...
// Disable invokes the DOM method. Disables DOM agent for the given
// page.
...
// DiscardSearchResults invokes the DOM method. Discards search
// results from the session with the given id. `getSearchResults`
...
// Enable invokes the DOM method. Enables DOM agent for the given
// page.
...
// GetAttributes invokes the DOM method. Returns attributes for the
// specified node.
...
// GetBoxModel invokes the DOM method. Returns boxes for the given
// node.
...
// GetFlattenedDocument invokes the DOM method. Returns the root DOM
// node (and optionally the subtree) to the caller.
...
// GetRelayoutBoundary invokes the DOM method. Returns the id of the
// nearest ancestor that is a relayout boundary.
...
// GetSearchResults invokes the DOM method. Returns search results
// from given `fromIndex` to given `toIndex` from the search with the
// given identifier.
...
// MarkUndoableState invokes the DOM method. Marks last undoable
// state.
...
// MoveTo invokes the DOM method. Moves node into the new container,
// places it before the given anchor.
...
// PerformSearch invokes the DOM method. Searches for a given string
// in the DOM tree. Use `getSearchResults` to access search results or
...
// PushNodeByPathToFrontend invokes the DOM method. Requests that the
// node is sent to the caller given its path. // FIXME, use XPath
...
// PushNodesByBackendIdsToFrontend invokes the DOM method. Requests
// that a batch of nodes is sent to the caller given their backend node
// ids.
...
// QuerySelector invokes the DOM method. Executes `querySelector` on a
// given node.
...
// QuerySelectorAll invokes the DOM method. Executes
...
// RemoveAttribute invokes the DOM method. Removes attribute with
// given name from an element with given id.
...
// RequestChildNodes invokes the DOM method. Requests that children of
// the node with given id are returned to the caller in form of
// `setChildNodes` events where not only immediate children are
...
// RequestNode invokes the DOM method. Requests that the node is sent
// to the caller given the JavaScript node object reference. All nodes
// that form the path from the node to the root are also sent to the
// client as a series of `setChildNodes` notifications.
...
// ResolveNode invokes the DOM method. Resolves the JavaScript node
// object for a given NodeId or BackendNodeId.
...
// SetAttributeValue invokes the DOM method. Sets attribute for an
...
// SetAttributesAsText invokes the DOM method. Sets attributes on
// element with given id. This method is useful when user edits some
// existing attribute value and types in several attribute name/value
// pairs.
...
// SetFileInputFiles invokes the DOM method. Sets files for the given
// file input element.
...
// SetInspectedNode invokes the DOM method. Enables console to refer
// to the node with given id via $x (see Command Line API for more
// details $x functions).
...
// SetNodeName invokes the DOM method. Sets node name for a node with
// given id.
...
// SetNodeValue invokes the DOM method. Sets node value for a node
// with given id.
...
// SetOuterHTML invokes the DOM method. Sets node HTML markup, returns
// new node id.
