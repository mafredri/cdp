...
// Package dom implements the DOM domain. This domain exposes DOM read/write operations. Each DOM Node is represented with its mirror object
...
// domainClient is a client for the DOM domain. This domain exposes DOM read/write operations. Each DOM Node is represented with its mirror object
// that has an `id`. This `id` can be used to get additional information on the Node, resolve it into
// the JavaScript object wrapper, etc. It is important that client receives DOM events only for the
// nodes that are known to the client. Backend keeps track of the nodes that were sent to the client
// and never sends the same node twice. It is client's responsibility to collect information about
...
// CopyTo invokes the DOM method. Creates a deep copy of the specified node and places it into the target container before the
// given anchor.
...
// DescribeNode invokes the DOM method. Describes node given its id, does not require domain to be enabled. Does not start tracking any
...
// DiscardSearchResults invokes the DOM method. Discards search results from the session with the given id. `getSearchResults` should no longer
...
// GetSearchResults invokes the DOM method. Returns search results from given `fromIndex` to given `toIndex` from the search with the given
// identifier.
...
// PerformSearch invokes the DOM method. Searches for a given string in the DOM tree. Use `getSearchResults` to access search results or
...
// RequestChildNodes invokes the DOM method. Requests that children of the node with given id are returned to the caller in form of
...
// RequestNode invokes the DOM method. Requests that the node is sent to the caller given the JavaScript node object reference. All
...
// SetAttributesAsText invokes the DOM method. Sets attributes on element with given id. This method is useful when user edits some existing
...
// SetInspectedNode invokes the DOM method. Enables console to refer to the node with given id via $x (see Command Line API for more details
// $x functions).
