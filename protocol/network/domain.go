...
// Package network implements the Network domain. Network domain
// allows tracking network activities of the page. It exposes
// information about http, file, data and other requests and responses,
...
// domainClient is a client for the Network domain. Network domain
// allows tracking network activities of the page. It exposes
// information about http, file, data and other requests and responses,
// their headers, bodies, timing, etc.
...
// CanClearBrowserCache invokes the Network method. Tells whether
// clearing browser cache is supported.
...
// CanClearBrowserCookies invokes the Network method. Tells whether
// clearing browser cookies is supported.
...
// CanEmulateNetworkConditions invokes the Network method. Tells
// whether emulation of network conditions is supported.
...
// ClearBrowserCookies invokes the Network method. Clears browser
// cookies.
...
// Network.requestIntercepted which either modifies the request to
// continue with any modifications, or blocks it, or completes it with
// the provided response bytes. If a network fetch occurs as a result
// which encounters a redirect an additional Network.requestIntercepted
...
// DeleteCookies invokes the Network method. Deletes browser cookies
// with matching name and url or domain/path pair.
...
// Disable invokes the Network method. Disables network tracking,
...
// EmulateNetworkConditions invokes the Network method. Activates
...
// Enable invokes the Network method. Enables network tracking,
...
// GetAllCookies invokes the Network method. Returns all browser
// cookies. Depending on the backend support, will return detailed
...
// GetCookies invokes the Network method. Returns all browser cookies
// for the current URL. Depending on the backend support, will return
...
// GetResponseBody invokes the Network method. Returns content served
...
// GetResponseBodyForInterception invokes the Network method. Returns
// content served for the given currently intercepted request.
...
// parameters should be identical: method, url, async, request body,
...
// SearchInResponseBody invokes the Network method. Searches for given
...
// SetBlockedURLs invokes the Network method. Blocks URLs from
// loading.
...
// SetBypassServiceWorker invokes the Network method. Toggles ignoring
// of service worker for each request.
...
// SetCacheDisabled invokes the Network method. Toggles ignoring cache
// for each request. If `true`, cache will not be used.
...
// SetCookie invokes the Network method. Sets a cookie with the given
// cookie data; may overwrite equivalent cookies if they exist.
...
// SetExtraHTTPHeaders invokes the Network method. Specifies whether
// to always send extra HTTP headers with the requests from this page.
...
// SetRequestInterception invokes the Network method. Sets the
// requests to intercept that match a the provided patterns and
...
// SetUserAgentOverride invokes the Network method. Allows overriding
// user agent with the given string.
