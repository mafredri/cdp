...
// Package target implements the Target domain. Supports additional
// targets discovery and allows to attach to them.
package target
...
// domainClient is a client for the Target domain. Supports additional
// targets discovery and allows to attach to them.
...
// ActivateTarget invokes the Target method. Activates (focuses) the
// target.
...
// AttachToTarget invokes the Target method. Attaches to the target
// with given id.
...
// CloseTarget invokes the Target method. Closes the target. If the
// target is a page that gets closed too.
...
// BrowserContext. Similar to an incognito profile but you can have
// more than one.
...
// DetachFromTarget invokes the Target method. Detaches session with
// given id.
...
// DisposeBrowserContext invokes the Target method. Deletes a
// BrowserContext, will fail of any open page uses it.
...
// GetTargetInfo invokes the Target method. Returns information about
// a target.
...
// SendMessageToTarget invokes the Target method. Sends protocol
// message over session with given id.
...
// SetAutoAttach invokes the Target method. Controls whether to
// automatically attach to new targets which are considered to be
// related to this one. When turned on, attaches to all existing
// related targets as well. When turned off, automatically detaches
...
// SetDiscoverTargets invokes the Target method. Controls whether to
// discover available targets and notify via
...
// SetRemoteLocations invokes the Target method. Enables target
// discovery for the specified locations, when `setDiscoverTargets` was
// set to `true`.
