...
// Package debugger implements the Debugger domain. Debugger domain
// exposes JavaScript debugging capabilities. It allows setting and
// removing breakpoints, stepping through execution, exploring stack
// traces, etc.
package debugger
...
// domainClient is a client for the Debugger domain. Debugger domain
// exposes JavaScript debugging capabilities. It allows setting and
// removing breakpoints, stepping through execution, exploring stack
// traces, etc.
...
// ContinueToLocation invokes the Debugger method. Continues execution
// until specific location is reached.
...
// Disable invokes the Debugger method. Disables debugger for given
// page.
...
// Enable invokes the Debugger method. Enables debugger for the given
// page. Clients should not assume that the debugging has been enabled
...
// EvaluateOnCallFrame invokes the Debugger method. Evaluates
...
// GetPossibleBreakpoints invokes the Debugger method. Returns
// possible locations for breakpoint. scriptId in start and end range
...
// GetScriptSource invokes the Debugger method. Returns source for the
// script with given id.
...
// GetStackTrace invokes the Debugger method. Returns stack trace with
// given `stackTraceId`.
...
// Pause invokes the Debugger method. Stops on the next JavaScript
// statement.
...
// RestartFrame invokes the Debugger method. Restarts particular call
...
// Debugger.pauseOnAsyncTask instead. Steps into next scheduled async
// task if any is scheduled before next pause. Returns success when
// async task is actually scheduled, returns error if no task were
...
// SearchInContent invokes the Debugger method. Searches for given
// string in script content.
...
// SetAsyncCallStackDepth invokes the Debugger method. Enables or
...
// SetBlackboxPatterns invokes the Debugger method. Replace previous
// blackbox patterns with passed ones. Forces backend to skip
// stepping/pausing in scripts with url matching one of the patterns.
// VM will try to leave blackboxed script by performing 'step in'
...
// SetBlackboxedRanges invokes the Debugger method. Makes backend skip
// steps in the script in blackboxed ranges. VM will try leave
// blacklisted scripts by performing 'step in' several times, finally
...
// SetBreakpoint invokes the Debugger method. Sets JavaScript
...
// SetBreakpointByURL invokes the Debugger method. Sets JavaScript
// breakpoint at given location specified either by URL or URL regex.
...
// SetBreakpointsActive invokes the Debugger method. Activates /
...
// exceptions state. Can be set to stop on all exceptions, uncaught
// exceptions or no exceptions. Initial pause on exceptions state is
// `none`.
...
// SetReturnValue invokes the Debugger method. Changes return value in
// top frame. Available only at return break position.
...
// SetScriptSource invokes the Debugger method. Edits JavaScript
// source live.
...
// SetSkipAllPauses invokes the Debugger method. Makes page not
// interrupt on any pauses (breakpoint, exception, dom exception etc).
...
// SetVariableValue invokes the Debugger method. Changes value of
// variable in a callframe. Object-based scopes are not supported and
...
// StepOut invokes the Debugger method. Steps out of the function
// call.
