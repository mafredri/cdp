...
// Package debugger implements the Debugger domain. Debugger domain exposes JavaScript debugging capabilities. It allows setting and removing
...
// domainClient is a client for the Debugger domain. Debugger domain exposes JavaScript debugging capabilities. It allows setting and removing
// breakpoints, stepping through execution, exploring stack traces, etc.
...
// Enable invokes the Debugger method. Enables debugger for the given page. Clients should not assume that the debugging has been
...
// GetPossibleBreakpoints invokes the Debugger method. Returns possible locations for breakpoint. scriptId in start and end range locations should be
// the same.
...
// ScheduleStepIntoAsync invokes the Debugger method. This method is deprecated - use Debugger.stepInto with breakOnAsyncCall and
...
// SetBlackboxPatterns invokes the Debugger method. Replace previous blackbox patterns with passed ones. Forces backend to skip stepping/pausing in
...
// SetBlackboxedRanges invokes the Debugger method. Makes backend skip steps in the script in blackboxed ranges. VM will try leave blacklisted
...
// SetBreakpointByURL invokes the Debugger method. Sets JavaScript breakpoint at given location specified either by URL or URL regex. Once this
...
// SetPauseOnExceptions invokes the Debugger method. Defines pause on exceptions state. Can be set to stop on all exceptions, uncaught exceptions or
...
// SetVariableValue invokes the Debugger method. Changes value of variable in a callframe. Object-based scopes are not supported and must be
// mutated manually.
