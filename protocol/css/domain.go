...
// Package css implements the CSS domain. This domain exposes CSS read/write operations. All CSS objects (stylesheets, rules, and styles)
...
// domainClient is a client for the CSS domain. This domain exposes CSS read/write operations. All CSS objects (stylesheets, rules, and styles)
// have an associated `id` used in subsequent operations on the related object. Each object type has
// a specific `id` structure, and those are not interchangeable between objects of different kinds.
// CSS objects can be loaded using the `get*ForNode()` calls (which accept a DOM node id). A client
// can also keep track of stylesheets via the `styleSheetAdded`/`styleSheetRemoved` events and
...
// AddRule invokes the CSS method. Inserts a new rule with the given `ruleText` in a stylesheet with given `styleSheetId`, at the
...
// Enable invokes the CSS method. Enables the CSS agent for the given page. Clients should not assume that the CSS agent has been
...
// ForcePseudoState invokes the CSS method. Ensures that the given node will have specified pseudo-classes whenever its style is computed by
// the browser.
...
// GetInlineStylesForNode invokes the CSS method. Returns the styles defined inline (explicitly in the "style" attribute and implicitly, using DOM
...
// GetPlatformFontsForNode invokes the CSS method. Requests information about platform fonts which we used to render child TextNodes in the given
// node.
...
// SetEffectivePropertyValueForNode invokes the CSS method. Find a rule with the given active property for the given node and set the new value for this
// property
...
// TakeCoverageDelta invokes the CSS method. Obtain list of rules that became used since last call to this method (or since start of coverage
// instrumentation)
