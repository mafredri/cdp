...
// Package css implements the CSS domain. This domain exposes CSS
// read/write operations. All CSS objects (stylesheets, rules, and
// styles) have an associated `id` used in subsequent operations on the
// related object. Each object type has a specific `id` structure, and
// those are not interchangeable between objects of different kinds.
// CSS objects can be loaded using the `get*ForNode()` calls (which
// accept a DOM node id). A client can also keep track of stylesheets
// via the `styleSheetAdded`/`styleSheetRemoved` events and
...
// domainClient is a client for the CSS domain. This domain exposes
// CSS read/write operations. All CSS objects (stylesheets, rules, and
// styles) have an associated `id` used in subsequent operations on the
// related object. Each object type has a specific `id` structure, and
// those are not interchangeable between objects of different kinds.
// CSS objects can be loaded using the `get*ForNode()` calls (which
...
// `ruleText` in a stylesheet with given `styleSheetId`, at the
...
// CollectClassNames invokes the CSS method. Returns all class names
// from specified stylesheet.
...
// Disable invokes the CSS method. Disables the CSS agent for the
// given page.
...
// Enable invokes the CSS method. Enables the CSS agent for the given
// page. Clients should not assume that the CSS agent has been enabled
// until the result of this command is received.
...
// ForcePseudoState invokes the CSS method. Ensures that the given
// node will have specified pseudo-classes whenever its style is
...
// GetComputedStyleForNode invokes the CSS method. Returns the
...
// GetInlineStylesForNode invokes the CSS method. Returns the styles
// defined inline (explicitly in the "style" attribute and implicitly,
// using DOM attributes) for a DOM node identified by `nodeId`.
...
// GetMatchedStylesForNode invokes the CSS method. Returns requested
// styles for a DOM node identified by `nodeId`.
...
// GetMediaQueries invokes the CSS method. Returns all media queries
// parsed by the rendering engine.
...
// GetPlatformFontsForNode invokes the CSS method. Requests
// information about platform fonts which we used to render child
...
// GetStyleSheetText invokes the CSS method. Returns the current
// textual content and the URL for a stylesheet.
...
// SetEffectivePropertyValueForNode invokes the CSS method. Find a
// rule with the given active property for the given node and set the
...
// SetKeyframeKey invokes the CSS method. Modifies the keyframe rule
// key text.
...
// SetStyleSheetText invokes the CSS method. Sets the new stylesheet
// text.
...
// SetStyleTexts invokes the CSS method. Applies specified style edits
// one after another in the given order.
...
// StopRuleUsageTracking invokes the CSS method. The list of rules
// with an indication of whether these were used
...
// TakeCoverageDelta invokes the CSS method. Obtain list of rules that
// became used since last call to this method (or since start of
