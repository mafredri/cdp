...
// DomainHeader defines the entire domain in the cdp package.
func (g *Generator) DomainHeader(d proto.Domain) {
	g.hasContent = true
...
%[5]s
`, d.Name(), d.Desc(), interfaceDef.buf.String(), strings.ToLower(d.Name()), domDef.buf.String(), quotedImports(g.imports))
}
